...
class Bot {
  constructor(options) {
    ...
    this.discord = new eris(this.discordToken, { autoreconnect: true });
    
    this.logFile = options.logFile;
    this.logFileJson = options.logFileJson;
    this.wnl = new (winston.Logger)({
        transports: [
            new (winston.transports.File)({
            filename: this.logFile,
            json: this.logFileJson,
          })
        ]
    });
    this.avoidedIrcBot = options.avoidedIrcBot;
  }
  ...
  attachListeners() {
    ...
    this.ircClient.on('registered', (message) => {
      ...
      this.wnl.info('registered', {"text":message});
    ...
    this.discord.on('disconnect', (disonnect) => {
      logger.error('Received disconnect event from Discord. ', disconnect);
    });
    ...
    this.ircClient.on('nick', (oldNick, newNick, channels) => {
      this.wnl.info('nick', {"user": nick, "text": newNick});
    ...
    this.ircClient.on('join', (channelName, nick) => {
      ...
      this.wnl.info('join', {"user": nick, "channel": channelName});
      if (!this.ircStatusNotices) return;
    ...
    this.ircClient.on('part', (channelName, nick, reason) => {
      ...
      this.wnl.info('part', {"user": nick, "reason":reason, "channel": channelName});
      if (!this.ircStatusNotices) return;
      const channel = channelName.toLowerCase();
    ...
    this.ircClient.on('quit', (nick, reason, channels) => {
      ...
      this.wnl.info('quit', {"user": nick, "reason":reason, "channel": channels});
    ...
    this.ircClient.on('names', (channelName, nicks) => {
      ...
      this.wnl.info('names', {"user": Object.keys(nicks), "channel": channelName});
      if (!this.ircStatusNotices) return;
      const channel = channelName.toLowerCase();
    ...
    this.ircClient.on('invite', (channel, from) => {
      ...
      this.wnl.info('invite', {"from": from, "channel": channel});
 ...
 //
  static getDiscordNicknameOnServer(user) {
    return user.username;
  }
// */
/* //
  static getDiscordNicknameOnServer(user, guild) {
    const userDetails = guild.members.filter(user.id);
    if (userDetails) {
...
// */
  parseText(message) {
    ...
    return text
      ...
      .replace(/<@&(\d+)>/g, (match, roleId) => {
        const role = message.guild.roles.filter(roleId);
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        ...
        this.wnl.info('message', {"user":author.username,"disid":author.id,"text":text,"channel":ircChannel});
        ...
        if (text !== '') {
          ...
          this.wnl.info('message', {"user":author.username,"disid":author.id,"text":patternMap.text,"channel":ircChannel});
          this.ircClient.say(ircChannel, text);
        }
        ...
        if (message.attachments) {
          message.attachments.forEach((a) => {
            ...
            this.wnl.info('message', {"user":author.username,"disid":author.id,"text":patternMap.attachmentURL,"channel":ircChannel});
  ...
  findDiscordChannel(ircChannel) {
    ...
    if (discordChannelName) {
      ...
      const discordChannel = discordChannelName.startsWith('#') ? this.discord.groupChannels
        .filter(c => c.type === 'text')
        .find('name', discordChannelName.slice(1)) : this.discord.getChannel(discordChannelName);
  ...
  sendToDiscord(author, channel, text) {
      ...
      var oriauthor=author;
      if (!discordChannel) return;
      var avoidedIrcBotRegex = new RegExp(this.avoidedIrcBot);
      var botted = (avoidedIrcBotRegex.test(author) && (text.length>3) && (text.indexOf("<") != -1) && (text.indexOf(">") != -1));
      if (botted) {
        // Convert text formatting (bold, italics, underscore) and author
        author = stripColorsAndStyle(text.slice(text.indexOf("<")+1,text.indexOf(">")).replace(/[[:cntrl:]](\d+,|)\d+/,"")).replace("\\u00[0-9A-F]{2,2}","");
        var withFormat = formatFromIRCToDiscord(text.replace(/<.*?>\:? ?/,''));
        if (!withFormat) {
          withFormat=" ";
        }
        if (!author) {
        ...
        var withFormat = formatFromIRCToDiscord(text);
      }
      const patternMap = {
        author,
        nickname: author,
      ...
      if (this.isCommandMessage(text)) {
        ...
        if (botted) {
          this.wnl.info('message', {"user":author, "text":withFormat, "channel":channel,"bot":oriauthor});
        } else {
          this.wnl.info('message', {"user":author, "text":withFormat, "channel":channel});
        }
        if (this.formatCommandPrelude) {
          const prelude = Bot.substitutePattern(this.formatCommandPrelude, patternMap);
          discordChannel.createMessage(prelude);
        }
        discordChannel.createMessage(text);
        return;
      }
      ...
      const withMentions = withFormat.replace(/@[^\s]+\b/g, (match) => {
        ...
        const nickUser = guild.members.find(search);
        if (nickUser) {
          return nickUser;
        }

        const user = /*this.discord.users*/guild.members.find('username', search);
        if (user) {
          return user;
        }
      ...
      discordChannel.createMessage(withAuthor);
      }
  ...
  sendExactToDiscord(channel, text) {
    ...
    this.wnl.info('exact', {"text":text, "channel":channel});
    discordChannel.createMessage(text);
  }
}
